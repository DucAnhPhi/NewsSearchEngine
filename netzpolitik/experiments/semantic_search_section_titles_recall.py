...
if __name__ == "__main__":
    # Semantic search section titles recall avg: 0.06876687001790294
    ...
    if os.path.isfile(f"{storage_location}/data/storage_section_titles.bin"):
        print("Loading vector storage from file...\n")
        vs = VectorStorage(f"{storage_location}/data/storage_section_titles.bin")
    else:
        print("Initialize vector storage.\n")
        vs = VectorStorage()
        print("Add items from file...\n")
        vs.add_items_from_file(f"{storage_location}/data/netzpolitik.jsonl", fe.get_titles_w_section_titles_embedding)
        vs.save(f"{storage_location}/data/storage_section_titles.bin")
    # build query
    with open(f"{storage_location}/data/judgement_list_netzpolitik.jsonl", "r") as f:
        for line_i, line in enumerate(f):
            ...
            try:
                ...
                query = fe.get_titles_w_section_titles_embedding(query_article)
    ...
    print(f"Semantic search section titles recall avg: {recall_avg}")
